...
class IntentPredictor:
    def __init__(self, data_path, language, num_speakers, num_clusters_per_stage):
        ...
        if self.num_speakers > 2 or self.num_speakers < 1:
            ...
            self.clusters = ClustersOneSpeaker(self.data_path, language, first_stage_num_clusters, self.second_stage_num_clusters)
        else:
            self.clusters = ClustersTwoSpeakers(self.data_path, language, first_stage_num_clusters, self.second_stage_num_clusters)
    ...
    def one_partite_dgl_graphs_preprocessing(self):
        ...
        valid_x, valid_y = get_data_one_speaker(self.clusters.valid_dataset, self.top_k, self.second_stage_num_clusters, 
    ...
    def two_partite_dgl_graphs_preprocessing(self):
        user_train_x, user_train_y, sys_train_x, sys_train_y = get_data_two_speakers(
                ...
                self.top_k, self.second_stage_num_clusters, 
                self.clusters.train_user_df, self.clusters.train_system_df,
        ...
        user_valid_x, user_valid_y, sys_valid_x, sys_valid_y = get_data_two_speakers(
                ...
                self.top_k, self.second_stage_num_clusters, 
                self.clusters.valid_user_df, self.clusters.valid_system_df,
    ...
    def train_model(self, train_dataloader, valid_dataloader, model_file_name,
                    early_stopping_steps = 5,  hidden_dim = 512, 
                    num_heads = 2, learning_rate = 0.0001, num_epochs = 100):
        
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        model = GAT_model(self.clusters.embs_dim, hidden_dim, num_heads, self.top_k, self.second_stage_num_clusters).to(device)
